// Fetch event JSON from GitHub Pages repo path
fetch('/CalAdd/data/event.json')
  ...
  .then(event => {
    ...
    // Calendar links
    const startTime = new Date(`${event.date} ${event.time.split('–')[0].trim()}`).toISOString().replace(/-|:|\.\d+/g,'');
    const endTime = new Date(`${event.date} ${event.time.split('–')[1].trim()}`).toISOString().replace(/-|:|\.\d+/g,'');
